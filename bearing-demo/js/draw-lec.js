d3.csv('../../data/lec.csv').then(function(data) {
  dataAll = data.filter(function(d){ return d.loss_event == 'All' })
  ...
  function getColor(d) {
      var p = d3.scaleOrdinal()
          ...
          .domain(['All','Ransomware', 'Business Email Compromise', 'Data Exfiltration'])
        return p(d);
      }
  function getLoss(d) {
      var p = d3.scaleOrdinal()
          .range([315000, 220000, 80000, 15000])
          .domain(['All','Ransomware', 'Business Email Compromise', 'Data Exfiltration'])
        return p(d);
          }
  function getChange(d) {
      var p = d3.scaleOrdinal()
          .range([35000, -40000, 20000, -10000])
          .domain(['All','Ransomware', 'Business Email Compromise', 'Data Exfiltration'])
        return p(d);
          }
  ...
  var tooltip = d3.tip()
    ...
    + formatAUD(lossAccessor(d)) + "</b> in the next year from <b>all events</b>"
    )
    .direction('n');
